...
if($_SERVER["REQUEST_METHOD"] == "POST" and $_POST['submit']!="")
{
		...
		if($_POST['regLicence']!="")
		{
			$regLicence=$_POST['regLicence'];

			$orgLicence_sql="select * from organization where regLicence=".$regLicence;
			$orgLicence_result=mysql_query($orgLicence_sql);
			$orgLicence_count=mysql_num_rows($orgLicence_result);
			if($orgLicence_count!=0)
			{
				$orgLicence_row=mysql_fetch_array($orgLicence_result);
				$orgKey=$orgLicence_row['organizationKey'];
				$contactEmail=$orgLicence_row['contactEmail'];
					...
					$mail->Subject = "秉信成金融服务-新企业接入确认邮件";
					$mail->Body = "尊敬的用户，<br/>
									您好！<br/>
									恭喜您，您的企业信息已经通过验证并成功接入秉信成金融系统。从今天起，我们的服务会让您的贷款管理更加便捷和效率。<br/>
									您的公司唯一验证码如下:".$orgKey."。此验证码用于贵企业的员工注册成为我们系统的会员，请妥善保存，切勿遗失和泄密。<br/>
									再次感谢您对于我们的支持！<br/>
										您的第三方金融服务管家 南京秉信成";
					...
					if(!$mail->Send()){
						...
						$send_status=1;
						$send_result="恭喜！您的企业信息已通过验证。确认邮件已成功发送到您的邮箱。";
					} 
				}
				else
				{
					$send_status=0;
					$send_result="抱歉，您的企业注册号不存在或者我们的工作人员正在确认您的企业信息。";
				}
		}
		else
		{
			$send_status=0;
			$send_result="*企业注册号尚未填写，请补充资料，谢谢！";
		}
}
?>
		...
		<div class="bdd">
				...
				<div class="user-lr-box-left f_l" style="width:590px">
					<div class="user_inc_tip mb15 pb15">用户可以在网站上获得企业信用评级、发布借款请求满足企业的资金需要。</div>
									...
									<div class="field mobile pr">
										...
										<span class="hint"> 请输入您企业的注册号，如果企业已经成功接入，系统会立即给您发送确认邮件和企业唯一注册码。</span>
									</div>
            ...
            <?php
				if($send_status==1)
				{
					...
					echo '<center><label style="color:#C00"><h2>'.$send_result.'</h2></label></center>';			
			?>	    
					</div>
