...
<div class="long_uc f_l">
	...
	<ul>
		<li class="current"><a href="/index.php?ctl=uc_deal&act=borrowed">已投保贷款</a></li>
	</ul>
</div>
...
<div id="borrowed" class="tc">
	<?php
	$loan_query="select * from t_loan where insuranceID=".$orgID;
	$loan_result=mysql_query($loan_query);
	$loan_number=mysql_num_rows($loan_result);
	while($loan_row = mysql_fetch_array($loan_result))
		{
			$loanorg_sql="select * from organization where organizationID=".$loan_row['organizationID'];
			$loanorg_result=mysql_query($loanorg_sql);
			$loanorg_row=mysql_fetch_array($loanorg_result);
			$loanorgName=$loanorg_row['orgName'];
			if($loan_row['bankID']!=NULL)
			{
			...
			echo'
                ...
                <div class="f_l" style="width:100px;height: 64px;">
                    <a href="view_org_profile.php?orgID='.$loan_row['organizationID'].'"; target="_blank">'.$loanorgName.'</a>
                </div>
				...
				<div class="f_l tc" style="width:60px">
					<a href="view_loan_payment.php?loanID='.$loanID.'&orgID='.$loan_row['organizationID'].'"; target="_blank">更多</a>
															</div>
            </div>
