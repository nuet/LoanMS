...
if($_SERVER["REQUEST_METHOD"] == "POST" and $_POST['submit']!="")
	{
		...
		if($_POST['newfeeTotal']!="" and $_POST['dueyear']!="" and $_POST['duemonth']!="" and $_POST['dueday']!="" and $_POST['payyear']!="" and $_POST['paymonth']!="" and $_POST['payday']!="")
		{
			$newfeeTotal= mysql_real_escape_string($_POST['newfeeTotal']);
			...
			$loanFee_sql="INSERT INTO t_loan_fee SET feeTotal='".$newfeeTotal."', dueDate='".$newdueDate."', payDate='".$newpayDate."', loanID='".$loanID."';";
			
			$loanFee_query = mysql_query($loanFee_sql);
			if($loanFee_query)
			{
				$update_status=1;
				$update_result='贷款管理费缴费信息成功添加！请点击 <a href="loan_fee.php?loanID='.$loanID.'"; style="color:red"> 这里 </a> 刷新页面.';
        	}
        	else
        	{
				$update_status=0;
     		   	$update_result="对不起，贷款管理费缴费信息未成功添加。请重试。";
        	}    	
		}
		else
		{
	...
	<div id="dashboard" class="dashboard">
		<ul>
           ...
           <li ><a href="loan_mortgage.php?loanID=<?php echo $loanID;?>">贷款抵押/质押信息</a></li>
            <li class="current"><a href="loan_fee.php?loanID=<?php echo $loanID;?>">贷款管理费信息</a></li>
		</ul>
	</div>
	<div class="inc_main uc_r_bl_box clearfix pt5 pr15 pl15 pb5">
		...
		<div class="clearfix">
			<form method="post" action="" name="save_bind_notice">
	            <table width="100%" border="0" cellspacing="1px" class="funds" style="margin:10px auto">
	                <tbody>
		                <tr class="title f_dgray b">
		                    <td width="34%">管理费数目</td>
		                    <td width="34%">缴费截止日期 </td>
		                    <td width="33%">实际缴费日期</td>
		                </tr>
                        <?php
						$fee_sql="select * from t_loan_fee where loanID=".$loanID;
        		...
        		<tr>
                    <td width="50">
                        <div align="left">缴费数目：</div>
                    </td>
                    <td>
                        <input type="text" id="newfeeTotal" name="newfeeTotal" class="f-input">
                    </td>
