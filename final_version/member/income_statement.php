...
$statement_query='select * from t_income_statement where statementID='.$statementID;
$statement_result=mysql_query($statement_query);
$statement_row=mysql_fetch_array($statement_result);
?>
...
if($_SERVER["REQUEST_METHOD"] == "POST" and $_POST['submit']!="")
	{
		if($_POST['mainIncome']!="" and $_POST['netProfit']!="")
		{
			$statementDate=date('Y-m-d');
			$mainIncome=(float)mysql_real_escape_string($_POST['mainIncome']);
			$mainCost=(float)mysql_real_escape_string($_POST['mainCost']);
			$salesTax= (float)mysql_real_escape_string($_POST['salesTax']);
			$mainProfit= (float)mysql_real_escape_string($_POST['mainProfit']);
			$otherProfit= (float)mysql_real_escape_string($_POST['otherProfit']);
			$businessCost=(float)mysql_real_escape_string($_POST['businessCost']);
			$manageCost=(float)mysql_real_escape_string($_POST['manageCost']);
			$financeCost=(float)mysql_real_escape_string($_POST['financeCost']);
			$impairmentCost=(float)mysql_real_escape_string($_POST['impairmentCost']);
			$investGain=(float)mysql_real_escape_string($_POST['investGain']);
			$businessProfit=(float)mysql_real_escape_string($_POST['businessProfit']);
			$otherIncome=(float)mysql_real_escape_string($_POST['otherIncome']);
			$subsidyIncome=(float)mysql_real_escape_string($_POST['subsidyIncome']);
			$otherCost=(float)mysql_real_escape_string($_POST['otherCost']);
			$profitTotal=(float)mysql_real_escape_string($_POST['profitTotal']);
			$incomeTax=(float)mysql_real_escape_string($_POST['incomeTax']);
			$netProfit=(float)mysql_real_escape_string($_POST['netProfit']);
			$grossProfit=(float)mysql_real_escape_string($_POST['grossProfit']);
			$netMargin=(float)mysql_real_escape_string($_POST['netMargin']);
			$grossMargin=(float)mysql_real_escape_string($_POST['grossMargin']);
			
			
			$incomeUpdate="UPDATE t_income_statement SET statementDate='".$statementDate."',mainIncome='".$mainIncome."',mainCost='".$mainCost."',salesTax='".$salesTax."',mainProfit='".$mainProfit."', otherProfit='".$otherProfit."', businessCost='".$businessCost."', manageCost='".$manageCost."', financeCost='".$financeCost."', impairmentCost='".$impairmentCost."',investGain='".$investGain."',businessProfit='".$businessProfit."',otherIncome='".$otherIncome."',subsidyIncome='".$subsidyIncome."',otherCost='".$otherIncome."',profitTotal='".$profitTotal."',incomeTax='".$incomeTax."', netProfit='".$netProfit."', grossProfit='".$grossProfit."',netMargin='".$netMargin."',grossMargin='".$grossMargin."',submissionUser='".$userID."' where statementID=".$statementID;
			
			$incomeUpdate_query = mysql_query($incomeUpdate);
			if($incomeUpdate_query)
			{
				$update_status=1;
				$update_result='企业财务损益表成功更新！请点击 <a href="income_statement.php?statementID='.$statementID.'"; style="color:red"> 这里 </a> 刷新页面.';
        	}
        	else
        	{
				$update_status=0;
     		   	$update_result="对不起，企业财务损益表信息未成功更新。请重试。";
        	}    	
		}
		else
		{
				$update_status=0;
				$update_result="对不起，企业财务损益表信息不全。请重试。";
		}
}
?>
...
<div class="long_uc f_l">
	...
	<ul>
		<li class="current"><a href="">企业财务损益表更新</a></li>
	</ul>
</div>
...
<div class="blank"></div>
<div class="item_group" style="width: 650px">更新企业财务损益表  单位:万元</div>
				...
				<tr>
					<td class="pt10" style="width:460px">						
                        ...
                        <div class="field idno">
							...
							<input type="text" value="<?php echo $statement_row['startDate'];?>" readonly="readonly" disabled="disabled" class="f-input" id="startDate" name="startDate" size="30" onkeyup="idcheck(this);">
						</div>
                                  <div class="field idno">
							<label for="settings-idno"><span class="red">*</span>周期结束日期</label>
							<input type="text" value="<?php echo $statement_row['endDate'];?>" readonly="readonly" disabled="disabled" class="f-input" id="endDate" name="endDate" size="30" onkeyup="idcheck(this);">
						</div>
						<div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>主营业务收入</label>
							<input type="text" value="<?php echo $statement_row['mainIncome']?>" class="f-input" id="mainIncome" name="mainIncome" size="30" onkeyup="idcheck(this);" >
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>主营业务成本</label>
						  <input type="text" value="<?php echo $statement_row['mainCost']?>" id="mainCost" name="mainCost" size="30">
						</div>
						<div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>营业税及附加</label>
						  <input type="text" value="<?php echo $statement_row['salesTax']?>"  id="salesTax" name="salesTax" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>主营利润</label>
						  <input type="text" value="<?php echo $statement_row['mainProfit']?>"  id="mainProfit" name="mainProfit" size="30">
						</div>
						<div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>主营外其它利润</label>
						  <input type="text" value="<?php echo $statement_row['mainCost']?>"  id="otherProfit" name="otherProfit" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>营业费用</label>
						  <input type="text" value="<?php echo $statement_row['businessCost']?>"  id="businessCost" name="businessCost" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>管理费用</label>
						  <input type="text" value="<?php echo $statement_row['manageCost']?>"  id="manageCost" name="manageCost" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>财务费用</label>
						  <input type="text" value="<?php echo $statement_row['financeCost']?>"  id="financeCost" name="financeCost" size="30">
						</div>
                         <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>资产减值损失</label>
						   <input type="text" value="<?php echo $statement_row['impairmentCost']?>"  id="impairmentCost" name="impairmentCost" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>投资收益</label>
						  <input type="text" value="<?php echo $statement_row['investGain']?>"  id="investGain" name="investGain" size="30">
						</div>
						<div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>营业利润</label>
						  <input type="text" value="<?php echo $statement_row['businessProfit']?>"  id="businessProfit" name="businessProfit" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>营业外收入</label>
						  <input type="text" value="<?php echo $statement_row['otherIncome']?>"  id="otherIncome" name="otherIncome" size="30">
						</div>
						<div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>补贴收入</label>
						  <input type="text" value="<?php echo $statement_row['subsidyIncome']?>"  id="subsidyIncome" name="subsidyIncome" size="30">
						</div>                  
						<div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>营业外支出</label>
						  <input type="text" value="<?php echo $statement_row['otherCost']?>"  id="otherCost" name="otherCost" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>利润总额</label>
						  <input type="text" value="<?php echo $statement_row['profitTotal']?>"  id="profitTotal" name="profitTotal" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>所得税</label>
						  <input type="text" value="<?php echo $statement_row['incomeTax']?>"  id="incomeTax" name="incomeTax" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>净利润</label>
						  <input type="text" value="<?php echo $statement_row['netProfit']?>"  id="netProfit" name="netProfit" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>毛利润</label>
						  <input type="text" value="<?php echo $statement_row['grossProfit']?>"  id="grossProfit" name="grossProfit" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>净利率</label>
						  <input type="text" value="<?php echo $statement_row['netMargin']?>"  id="netMargin" name="netMargin" size="30">
						</div>
                        <div class="field mobile">
							<label for="settings-mobile"><span class="red">*</span>毛利率</label>
						  <input type="text" value="<?php echo $statement_row['grossMargin']?>"  id="grossMargin" name="grossMargin" size="30">
						</div>
				  </td>
