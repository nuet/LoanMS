...
<div class="long_uc f_l">
	...
	<ul>
		<li class="current"><a href="view_finance_manage_statement.php?orgID=<?php echo $orgID;?>">企业财务损益表管理</a></li>
        <li ><a href="view_finance_manage_sheet.php?orgID=<?php echo $orgID;?>">企业财务债务表管理</a></li>
        <li ><a href="view_credit_score.php?orgID=<?php echo $orgID;?>">企业信用估值历史</a></li>
	</ul>
</div>
...
<div class="silverBg tc pl10 " style="height:30px;line-height:30px; font-weight:bold; color:#8E8E8E">
    <div class="f_l tl" style="width:120px;text-indent:2em">
            管理链接
    </div>
...
<div id="borrowed" class="tc">
	<?php
	$statement_query="select * from t_income_statement where organizationID=".$orgID;
	$statement_result=mysql_query($statement_query);
	$statement_number=mysql_num_rows($statement_result);
	while($statement_row = mysql_fetch_array($statement_result))
		{
			$submission_sql="select * from users where userID=".$statement_row['submissionUser'];
			$submission_result=mysql_query($submission_sql);
			$submission_row=mysql_fetch_array($submission_result);
			$submissionName=$submission_row['LastName'].$submission_row['FirstName'];
			echo'
	            <div class="clearfix pb10">
                <div class="f_l" style="width:120px;height: 64px;" align="center">
                     <a href="view_income_statement.php?statementID='.$statement_row['statementID'].'&orgID='.$orgID.'">详情</a>
                </div>
                <div class="f_l tl" style="width:120px" align="center">
                    '.$statement_row['startDate'].'
                </div>
                <div class="f_l t1" style="width:120px;">
                	'.$statement_row['endDate'].'                
				</div>
                <div class="f_l t1" style="width:120px">
                    '.$statement_row['statementDate'].'               </div>
                <div class="f_l" style="width:120px">
                '.$submissionName.'
				</div>
            </div>
