...
if($_SERVER["REQUEST_METHOD"] == "POST" and $_POST['submit']!="")
	{
		...
		if($_POST['newpaymentTotal']!="" and $_POST['dueyear']!="" and $_POST['duemonth']!="" and $_POST['dueday']!="" and $_POST['payyear']!="" and $_POST['paymonth']!="" and $_POST['payday']!="")
		{
			$newpaymentTotal= mysql_real_escape_string($_POST['newpaymentTotal']);
			$newdueDate= mysql_real_escape_string($_POST['dueyear']).'-'.mysql_real_escape_string($_POST['duemonth'].'-'.mysql_real_escape_string($_POST['dueday']));
			$newpayDate= mysql_real_escape_string($_POST['payyear']).'-'.mysql_real_escape_string($_POST['paymonth'].'-'.mysql_real_escape_string($_POST['payday']));
			
			$loanPayment_sql="INSERT INTO t_loan_payment SET paymentTotal='".$newpaymentTotal."', dueDate='".$newdueDate."', payDate='".$newpayDate."', loanID='".$loanID."';";
			
			$loanPayment_query = mysql_query($loanPayment_sql);
			if($loanPayment_query)
			{
				$update_status=1;
				$update_result='贷款还款信息成功添加！请点击 <a href="loan_payment.php?loanID='.$loanID.'"; style="color:red"> 这里 </a> 刷新页面.';
        	}
        	else
        	{
				$update_status=0;
     		   	$update_result="对不起，贷款还款信息未成功添加。请重试。";
        	}    	
		}
		else
		{
...
<div class="inc wb">
	<div id="dashboard" class="dashboard">
		<ul>
			<li class="current"><a href="loan_payment.php?loanID=<?php echo $loanID;?>">贷款还款信息</a></li>
           <li ><a href="loan_mortgage.php?loanID=<?php echo $loanID;?>">贷款抵押/质押信息</a></li>
	...
	<div class="inc_main uc_r_bl_box clearfix pt5 pr15 pl15 pb5">
		...
		<div class="clearfix">
			<form method="post" action="" name="save_bind_notice">
	            <table width="100%" border="0" cellspacing="1px" class="funds" style="margin:10px auto">
	                <tbody>
		                <tr class="title f_dgray b">
		                    <td width="34%">还款数目</td>
		                    <td width="34%">还款截止日期 </td>
		                    <td width="33%">实际还款日期</td>
		                </tr>
                        <?php
						$payment_sql="select * from t_loan_payment where loanID=".$loanID;
						$payment_result=mysql_query($payment_sql);
						while($payment_row=mysql_fetch_array($payment_result))
						{
						echo '
		                <tr>
		                    <td class="pl10 pr10 b f_dgray tl">
							￥'.$payment_row['paymentTotal'].'
		                    </td>
		                    <td>
		                        '.$payment_row['dueDate'].'
		                    </td>
        		...
        		<tr>
                    <td width="50">
                        <div align="left">还款数目：</div>
                    </td>
                    <td>
                        <input type="text" id="newpaymentTotal" name="newpaymentTotal" class="f-input">
                    </td>
                ...
                <tr>
                    ...
                    <div class="blank10"></div>
                    <div class="blank10"></div>
                </tr><div class="tc"><input class="saveSettingBnt" type="submit" id="submit" name="submit" value="新还款信息"></div>
				<div class="blank20"></div>
			</form>
        </div>
	</div>
